...
session_start();
$doc = new DOMDocument();
$counter= intval($doc->getElementById('counter').value);
...
if($_SERVER['REQUEST_METHOD']=="POST" and isset($_POST['patientID']) and isset($_SESSION['counter'])) {
	...
	echo "<script>document.getElementById('counter').innerHTML = $temp_counter; </script>";
}
...
echo '<h1>Welcome to TelehealthIndia</h1>';
echo '<h2>Click below to view the list of files uploaded by you</h2><a class="btn btn-info btn-xs active" role="button" href="./uploadedfiles.php?gateway='.$_SESSION['gateway'].'&patientID='.$_SESSION['patientID'].'"><h3>Uploaded Files</h3></a> ';
echo '</div>';*/
?>
<div class = "row">
<div class = "col-md-12">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' and isset($_FILES['file'])) {
	...
	foreach($studyArray as $value) {
		if(!empty($value)) {
			$sql = 'SELECT study_iuid FROM member_study WHERE study_iuid="'.$value.'" AND gateway="'.$_SESSION['gateway'].'" AND pid="'.$_SESSION['patientID'].'"';
			mysql_select_db('uploaddicomdb');
			$retval = mysql_query( $sql, $conn );
			$row = mysql_fetch_array($retval, MYSQL_ASSOC);
			if(empty($row)) {
				$sql1 = "INSERT INTO member_study (gateway, pid, study_iuid) VALUES ('".$_SESSION['gateway']."','".$_SESSION['patientID']."','".$value."')";
...
$sql = "SELECT * FROM member_study WHERE gateway='".$_SESSION['gateway']."' AND pid='".$_SESSION['patientID']."'";
mysql_select_db('uploaddicomdb');
$retval = mysql_query( $sql, $conn );
if(! $retval )
{
...
while($row)
{
	...
	$sql1 = "SELECT * FROM study WHERE study_iuid='".$row['study_iuid']."'";
	...
	$study_url = 'http://'.$_SERVER['SERVER_NAME'].':8080/weasis/samples/applet.jsp?commands=%24dicom%3Aget%20-w%20http%3A//'.$_SERVER['SERVER_NAME'].'%3A8080/weasis-pacs-connector/manifest%3FstudyUID%3D'.$row['study_iuid'];
	echo '<td id = "cmnt'.$row['id'].'"><button id = "'.$row['id'].'" class="open-MyModal btn btn-primary btn-sm" data-toggle="modal" data-comment="'.$row['comments'].'" data-row-id="'.$row['id'].'" data-target="#myModal">Comments</button></td>';
